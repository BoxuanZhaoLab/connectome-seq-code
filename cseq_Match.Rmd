---
title: "new_match_Joe_010426.Rmd"
...
#' Selection is based on UMI_Count (frequency) and celltype information

select_best_match <- function(candidates) {
  # If only one candidate, return its index
    if (length(candidates$V2) == 1) return(c(1:length(candi)))
    # Find the highest UMI_Count among candidates
    max_count <- max(candidates$UMI_Count, na.rm = TRUE)
    # Select all candidates with the highest count
    best_counts <- candidates[candidates$UMI_Count == max_count,]
    ...
    if (nrow(best_counts) == 1) {
        ...
        return(which(candidates$UMI_Count == max_count)[1])
    ...
    if (sum(valid_celltype) == 1) {
        ...
        return(which(candidates$UMI_Count == max_count & 
...
process_sample_matches <- function(sample_id, pre_matches, post_matches,
    ...
    post_hamm <- post_final_match %>% 
        group_by(V2, match, V2.1, UMI_Count) %>% 
         summarize(synpcount = n(), .groups = "drop")
    ...
    filter_matches <- function(matches) {
        matches %>%
            ...
            group_by(V2) %>%
            mutate(max_count2 = max(UMI_Count)) %>%
            filter(UMI_Count == max_count2) %>%
            group_by(V2, V2.1) %>%
...
### This filter_neurons_new function is modified from filter_neuron function in new_match.
### Aim is to clean the nucleus barcode data base on UMI count rather than total count.
### Input data comes from the results of /home/zhouwan2/expansion_link/RNA_seq_processing/cseq1_revision_codes_082225/Nuc_UMI_clean_090925.ipynb, where CB-BC pairs are grouped and attached with UMI count.



filter_neurons_new <- function(sample_id, 
    ...
    pn_data <- fread(file.path(base_path, sprintf("cseq%d_PN.umi.csv", sample_id)), col.names = c("V1", "V2", "UMI_Count"))

    cn_data <- fread(file.path(base_path, sprintf("cseq%d_CN.umi.csv", sample_id)), col.names = c("V1", "V2", "UMI_Count"))
 ...
 # When saving the filtered data, explicitly name columns and drop the row index
    ...
    fwrite(pn_neurons, sprintf("cseq%d_PN.neurons.umi.csv", sample_id), row.names = FALSE)
    
    cn_neurons <- cn_data[cn_data$V2 %in% cn_neuron_barcodes]
    fwrite(cn_neurons, sprintf("cseq%d_CN.neurons.umi.csv", sample_id), row.names = FALSE)
...
disambiguate_assignments <- function(sample_id) {
    ...
    pn_neurons <- fread(sprintf("cseq%d_PN.neurons.umi.csv", sample_id))
    cn_neurons <- fread(sprintf("cseq%d_CN.neurons.umi.csv", sample_id))
    ...
    fwrite(pn_filtered, sprintf("cseq%d_PN.neuron.umiClean.csv", sample_id))
    fwrite(cn_filtered, sprintf("cseq%d_CN.neuron.umiClean.csv", sample_id))
...
process_nuc <- function(sample_ids = c(1, 25, 31, 34, 35, 36)) {
    ...
    for(sample_id in sample_ids) {
        ...
        neuron_results <- filter_neurons_new(sample_id)
    ...
    save(all_results, file = "disambiguated_nuc_barcodes_umi.RData")
    return(all_results)
}
results <- process_nuc()
...
process_all_samples <- function(sample_ids = c(1,25,31,34,35,36),
    ...
    for(sample_id in sample_ids) {
        ...
        tryCatch({
            ...
            pn_data <- fread(sprintf("cseq%d_PN.neuron.umiClean.csv", sample_id))
            cn_data <- fread(sprintf("cseq%d_CN.neuron.umiClean.csv", sample_id))

            
            # Load synapse data
            synp_pre <- fread(sprintf("cseq%d_synp.pre.clean.csv", sample_id))
            synp_post <- fread(sprintf("cseq%d_synp.post.clean.csv", sample_id))
            synp_pre$V2 <- gsub("cseq", "", synp_pre$V2)
            synp_post$V2 <- gsub("cseq", "", synp_post$V2)
...
analyze_connectome_cluster <- function(csv_filename = "connectome.match.count.csv") {
  ...
  data.raw <- read.table(file="connectome.match.count.csv", header=T, sep=",", row.names = 1)
...
analyze_connectome <- function(pre_data, post_data, output_csv = TRUE, 
  ...
  connectome.match.count <- connectome.match %>% 
    ...
    summarize(UMI_Count = sum(presynp_count, postsynp_count), .groups = 'drop')
  ...
  data.raw <- read.csv(csv_filename, row.names = 1)
  colnames(data.raw) <- c("syn","pre","post","celltype.pre","celltype.post","count")
...
generate_plots <- function(hamming_distances = 0:10, 
    ...
    for(hamm_dist in hamming_distances) {
        ...
        write.csv(connectome.match.count, "connectome.match.count.csv")
...
# Read the CSV data
data <- read.csv("./all_plots/hamm5/connectome.match.count.csv")
...
htmlwidgets::saveWidget(p, 
                       file = "sankey_diagram_092125.html", 
                       selfcontained = TRUE)
```
...
DimPlot(object = scdata.CB.5, cells.highlight = cseq.all.post.filtered$V2.1, cols.highlight = "#00A9FF", cols = "gray", order = TRUE, raster=FALSE)+coord_fixed()
